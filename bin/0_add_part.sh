...
while read line; do
...

  {
    ...
    while [[ ${f_time} -le $add_end_time ]]; do
      ...
      add_part+="ALTER TABLE ${dttb} ADD IF NOT EXISTS PARTITION (year_month='${ym_add}',day_of_month='${dm_add}'); ALTER TABLE ${dttb} DROP IF EXISTS PARTITION (year_month='${ym_del}',day_of_month='${dm_del}');"
