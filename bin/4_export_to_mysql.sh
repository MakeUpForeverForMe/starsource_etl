...
while read line; do
  ...
  [[ $? == 0 ]] && succ "Fields_Filter $hive_dbtable" '获取Hive字段成功' || erro "Fields_Filter $hive_dbtable" '获取Hive字段失败'
  ...
  [[ $? == 0 ]] && succ "Fields_Filter $hive_dbtable" '获取MySQL字段成功' || erro "Fields_Filter $hive_dbtable" '获取MySQL字段失败'
